...
namespace GeniusBar.Controllers
{
    public class UserRecycleOrdersController : ApiController
    {
        ...
        // GET: api/engineer/recycle
        [Route("api/engineer/recycle")]
        [HttpGet]
        [ResponseType(typeof(RecycleOrder))]
        public IHttpActionResult GetEngineerRecycleOrders()
        {
            User cookiedUser = getCooikedUser();

            Console.WriteLine(cookiedUser.ID);

            return Ok(db.RecycleOrders.Where(e => e.Engineer_ID == cookiedUser.ID).ToList<RecycleOrder>());
        }



        // GET: api/engineer/recycle
        [Route("api/engineer/recycle/{id}", Name = "GetEngineerRecycleOrder")]
        [HttpGet]
        [ResponseType(typeof(RecycleOrder))]
        public IHttpActionResult GetEngineerRecycleOrder(int id)
        {
            User cookiedUser = getCooikedUser();
            var recycleOrder = db.RecycleOrders.FirstOrDefault(u => (u.Engineer_ID == cookiedUser.ID) && (u.ID == id));
            if (recycleOrder == null)
            {
                return NotFound();
            }
        ...
        // GET: api/engineer/undispatchedRecycle
        [Route("api/engineer/undispatchedRecycle")]
        [HttpGet]
        [ResponseType(typeof(RecycleOrder))]
        public IHttpActionResult GetUndipatchedRecycleOrders()
        {
            return Ok(db.RecycleOrders.Where(e => e.Engineer_ID == null).ToList<RecycleOrder>());
        }
        ...
        // PUT: /api/engineer/recycleOrderState/{id}
        [Route("api/engineer/recycleOrderState/{id}")]
        [HttpPut]
        [ResponseType(typeof(RecycleOrder))]
        public IHttpActionResult PutRecycleOrderState(int id, EngineerUpdateData engineerUpdateData)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
