...
namespace GeniusBar.Controllers
{
    public class UserRepairOrdersController : ApiController
    {
        ...
        // GET: api/engineer/repair
        [Route("api/engineer/repair")]
        [HttpGet]
        [ResponseType(typeof(RepairOrder))]
        public IHttpActionResult GetEngineerRepairOrders()
        {
            User cookiedUser = getCooikedUser();

            Console.WriteLine(cookiedUser.ID);

            return Ok(db.RepairOrders.Where(e => e.Engineer_ID == cookiedUser.ID).ToList<RepairOrder>());
        }



        // GET: api/engineer/repair/{id}
        [Route("api/engineer/repair/{id}", Name = "GetEngineerRepairOrder")]
        [HttpGet]
        [ResponseType(typeof(RepairOrder))]
        public IHttpActionResult GetEngineerRepairOrder(int id)
        {
            User cookiedUser = getCooikedUser();
            var repairOrder = db.RepairOrders.FirstOrDefault(u => (u.Engineer_ID == cookiedUser.ID) && (u.ID == id));
            if (repairOrder == null)
            {
                return NotFound();
            }
        ...
        // GET: api/engineer/undispatchedRepair
        [Route("api/engineer/undispatchedRepair")]
        [HttpGet]
        [ResponseType(typeof(RepairOrder))]
        public IHttpActionResult GetUndipatchedRepairOrders()
        {
            return Ok(db.RepairOrders.Where(e => e.Engineer_ID == null).ToList<RepairOrder>());
        }


        public class EngineerUpdateData
        {
            public string Staff_note;
            public decimal Price;
            public RepairOrderState State;
        ...
        // PUT: /api/engineer/repairOrderState/{id}
        [Route("api/engineer/repairOrderState/{id}")]
        [HttpPut]
        [ResponseType(typeof(RepairOrder))]
        public IHttpActionResult PutRepairOrderState(int id, EngineerUpdateData engineerUpdateData)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            if (order.Engineer_ID != getCooikedUser().ID)
            {
                return Unauthorized();
            }

            order.Staff_note = engineerUpdateData.Staff_note;
            order.Price = engineerUpdateData.Price;
            order.State = engineerUpdateData.State;
